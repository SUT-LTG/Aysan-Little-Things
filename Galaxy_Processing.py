...
def background_romval(data, mesh, filter_size, name, save = False, info = False, n = 1):
    ...
    for j in range(len(background_removed_image)):
        for i in range(len(background_removed_image[0])):
            if background_removed_image[j][i] < median - n * sigma_bkg and background_removed_image[j][i] < 0:
                background_removed_image[j][i] = 0
    min_image = np.min(background_removed_image)
    if info:
        print("min image = ", min_image)
    if min_image < 0:
...
def fast_convolve(image, box_radius, sigma):
    ...
    box = box/np.sum(box)
    return convolve2d(image,box,'same')
...
def half_radius_ellipse(blurred_image, image, lim_list, name, linear_range = [0, 30], plot = True, fit = True):
    ...
    if fit:
        popt, pcov = curve_fit(line, lim_list[linear_range[0]:linear_range[1]], intensity_list[linear_range[0]:linear_range[1]])
    if plot:
        plt.figure()
        ...
        if fit:
            plt.plot(lim_list, line(lim_list, popt[0], popt[1]), label = "integrated intensity criteria")
            plt.plot(lim_list, line(lim_list, popt[0], popt[1] + np.sqrt(pcov[1][1])), label = "upper integrated intensity criteria")
            plt.plot(lim_list, line(lim_list, popt[0], popt[1] - np.sqrt(pcov[1][1])), label = "lower integrated intensity criteria")
    
    if fit:
        CI = intensity_list - line(lim_list, popt[0], popt[1])
        
        for i in range(len(lim_list)):
            if CI[i]  < np.sqrt(pcov[1][1]):
                radius_index = i
                break
...
def mag_25_radius(blurred_image, image, name, plot = True):
    ...
    return np.max(a, b), np.min(a, b), theta * 180 / np.pi, x_center, y_center
